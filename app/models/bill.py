...
# 🔥 prevent circular import issues while keeping Pylance happy
if TYPE_CHECKING:
...
class Bill(Base):
    ...
    paid_status: Mapped[bool] = mapped_column(
        Boolean,
        default=False,
        nullable=False
    )
    ...
    bookkeeper_auto: Mapped[bool] = mapped_column(
        Boolean,
        default=True,
        nullable=False
    )
    ...
    keeper: Mapped["Person | None"] = relationship(
        back_populates="kept_bills",
        foreign_keys=[keeper_id]
    )

    details: Mapped[List["BillDetail"]] = relationship(
        back_populates="bill",
